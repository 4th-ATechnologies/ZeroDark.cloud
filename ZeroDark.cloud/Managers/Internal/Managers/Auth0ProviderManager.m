								 ...
								 bucket:(NSString *)aws_bucket
{
	...
	BOOL isRecoveryId =  [Auth0Utilities isRecoveryProfile:profileData];
	if(isRecoveryId)
		return nil;
	...
	if(comps.count == 2)
	{
		...
		if([provider isEqualToString:A0StrategyNameAuth0]
		  &&(aws_bucket.length > 0) && (aws_region != AWSRegion_Invalid))
		{

			NSURL* avatarURL = [self avatarUrlForUserForAuth0ID:auth0ID
																	  region:aws_region
																	  bucket:aws_bucket];
			if(avatarURL)
				result = avatarURL.absoluteString;
		}
		else
		{
			...
			if(picture)
			{
				...
				if(url)
				{
					...
					// do fixes for the providers.
					if(picture)
					{
						// bitbucket needs icon fix
						if([provider isEqualToString:@"bitbucket"])
						{
							result = [picture stringByReplacingOccurrencesOfString:@"/32/" withString:@"/128/"];
						}
						else if([provider isEqualToString:@"facebook"])
						{
							result = [NSString stringWithFormat:@"https://graph.facebook.com/%@/picture", providerID];
						}
						else
						{
							result = picture;
