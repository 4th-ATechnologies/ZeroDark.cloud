...
@interface UIImage (BadgedBarButtonItem)
+ (CGFloat)pointsToPixels:(CGFloat)points;
@end


@implementation UIImage (BadgedBarButtonItem)
+ (CGFloat)pointsToPixels:(CGFloat)points {
...
@implementation BadgedBarButtonItem
{
	CATransition *transition;
	UIView 		*vwButton;
	UIImageView *imgImage;
	UILabel		*lblBadge;
	BadgedBarButtonItem_Offset mode;

	UITapGestureRecognizer *tap;

	BadgedBarButtonItemActionBlock  actionBlock;
}

-(id)initWithImage:(UIImage *)image
			  mode:(BadgedBarButtonItem_Offset)modeIn
	   actionBlock:(BadgedBarButtonItemActionBlock)actionBlockIn
{
...

{
	...
	BadgedBarLabel *label = [[BadgedBarLabel alloc] initWithFrame: (CGRect)
					  {
...
-(void)setBadgeText:(NSString *)text
{
	...
	else
	{
		...
		if(mode == kBadgedBarButtonItem_Offset_Top)
		{
		...
		else if(mode == kBadgedBarButtonItem_Offset_Center)
		{
			badgeFrame.origin.y =  (vwButton.frame.size.height/2) -  (imageFrame.size.height /2) ;
		}
		else if(mode == kBadgedBarButtonItem_Offset_Bottom)
		{
